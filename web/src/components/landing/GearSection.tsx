  ...
  return <section id="capture-gear" className="scroll-mt-24 border-y border-white/[.07] bg-[#141923] py-24 sm:py-32 lg:py-40">
    <div className={shell}>
      ...
      <div className="mt-12 grid gap-px overflow-hidden rounded-xl border border-white/10 bg-white/10 md:grid-cols-2 xl:grid-cols-4">
        {gear.map(([title, status, Icon], index) => <article key={title} className="min-h-[230px] bg-[var(--surface)] p-6"><div className="flex items-start justify-between"><span className="font-mono text-[11px] text-white/40">0{index + 1}</span><Icon aria-hidden="true" size={26} className="text-[var(--primary)]" /></div><h3 className="mt-16 font-heading text-xl leading-tight">{title}</h3><p className="mt-4 font-mono text-[10px] uppercase tracking-[.14em] text-[var(--primary)]">{status}</p></article>)}
      </div>
      <div className="mt-10"><PrimaryLink href="#join">Register for gear access</PrimaryLink></div>
    </div>
  </section>;
}
