...
const partners = [
  ["/digirobotics/partners/arbitrum.png", "Built with Arbitrum", "w-[112px]"],
  ["/digirobotics/partners/robinhood-chain.png", "Built with Robinhood Chain", "w-[108px]"],
  ["/digirobotics/partners/zerodev.png", "Built with ZeroDev", "w-[96px]"],
  ["/digirobotics/partners/paxos.png", "Built with Paxos", "w-[96px]"],
];

export function HeroSection() {
  return <section className="relative border-b border-white/[.07] pt-14 sm:pt-20 lg:pt-24">
    <div className={`${shell} grid items-center gap-12 lg:grid-cols-[1.16fr_.84fr] lg:gap-10`}>
      <Reveal>
        <Eyebrow>01 / Egocentric data for robotics</Eyebrow>
        <h1 className="mt-7 max-w-[920px] font-display text-[clamp(3.45rem,8.1vw,8.8rem)] font-bold uppercase leading-[.88] tracking-[-.065em]">
        ...
        <p className="mt-7 max-w-2xl text-[19px] leading-[1.5] text-[var(--muted-foreground)] lg:text-[21px]">Capture real-world skills from your point of view. Approved contributions become robotics training data—and contributors receive USDG or PYUSD after acceptance.</p>
      ...
      <Reveal delay={0.12} className="relative lg:translate-y-4">
        ...
        <div className="relative aspect-[8/6.15] overflow-hidden rounded-[28px] border border-white/10 bg-[var(--surface)] shadow-[var(--shadow-lift)]">
          <Image src="/digirobotics/hero/pov-thermostat.webp" alt="First-person view of hands installing a smart control panel" fill sizes="(max-width: 1024px) 100vw, 42vw" className="object-cover" priority />
    ...
    <div className={`${shell} mt-16 border-t border-white/[.07] py-7 lg:mt-24`}>
      <div className="flex flex-wrap items-center gap-x-8 gap-y-6">
        ...
        {partners.map(([src, alt, width]) => <div key={src} className="relative h-8 w-[120px]"><Image src={src} alt={alt} fill sizes="120px" className={`object-contain object-left ${width}`} /></div>)}
      </div>
    </div>
  </section>;
}
