  ...
  return <section id="custom-data" className="scroll-mt-24 py-24 sm:py-32 lg:py-40">
    <div className={shell}>
      ...
      <div className="mt-14 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 lg:grid-cols-3">
        {types.map(({ title, copy, icon: Icon }, index) => <a key={title} href="#request-form" className="group min-h-[260px] bg-[var(--surface)] p-7 transition duration-500 hover:bg-[#1c2430] sm:p-9">
          ...
          <h3 className="mt-16 font-heading text-3xl">{title}</h3><p className="mt-4 max-w-sm text-[17px] leading-6 text-[var(--muted-foreground)]">{copy}</p>
        </a>)}
      </div>
      <CustomDataForm />
    </div>
